...
class ShiftedNamingArguments:
    naming_dict: dict
    '''dictionary that holds naming arguments from shifted_args.json'''
...
class ShiftedFastaArguments:
    ...
    '''This toggle either creates every combination between all protein list generated by each json file if "combo" is entered in the json file,
    if "pair" is entered all chimera lists in the per json file will be paired one to one to one etc. or single file list will be created for homomeric proteins.
    Switch toggle to pair if you one of your unique proteins will not be a chimera'''
    ...
    def __init__(self,fasta_dict):
        ...
        with open(self.alignment_file_name, "r") as alignment:
            ...
            self.no_gap_ref_sequence = ''.join(x for ind, x in enumerate(sequence_dictionary[self.reference_identifier]) if x.isalpha())
...
for container in container_of_containers:
    ...
    if container.fasta_args.partner_identifier==container.fasta_args.reference_identifier:
        # if you dont want any of the proteins to be chimeras you can set the reference and partner fasta identifier to the same identifier
        container.add_chimera(chimeracls())
        boundary_info = alignment_finder(container.fasta_args.alignment_file_name, container.fasta_args.no_gap_ref_sequence,
...
if prime_container.fasta_args.fasta_toggles["Make pair or combo heteromers"] == "combo":
...
elif prime_container.fasta_args.fasta_toggles["Make pair or combo heteromers"] == "pair" or len(container_of_containers) == 1:
...
if prime_container.operation_toggles['run_analysis_operation?'] == 'True':
    ...
    for chimera_group in groupings:
        for chimera in chimera_group:
            if hasattr(chimera.container_id, 'not_chimera') and chimera.container_id.fasta_args.no_gap_ref_sequence in reference_plddts and chimera.container_id.fasta_args.no_gap_par_sequence in reference_plddts:
    ...
    savetxt(prime_container.analysis_args.analysis_output_file, data_array, fmt=','.join('%s' for x in data_columns), delimiter=",")
two = perf_counter()
print(two - one)

...
tprfile=["/gpfs/gpfs0/scratch/jws6pq/Gromacs/3merSARS2w" + x +"S1_production_1.tpr" for x in protein_list]
xtcfile=["/gpfs/gpfs0/scratch/jws6pq/Gromacs/3merSARS2w" + x +"S1_production_1.xtc" for x in protein_list]
timestep=['1000' for x in protein_list]
newxtcfile=["/gpfs/gpfs0/scratch/jws6pq/Gromacs/3merSARS2w" + x +"dt" + str(timestep[0]) +".xtc" for x in protein_list]
xvgfile=["/gpfs/gpfs0/scratch/jws6pq/Gromacs/3merSARS2w" + x +"dt" + str(timestep[0]) +".xvg" for x in protein_list]
averagedxvgfile=["/gpfs/gpfs0/scratch/jws6pq/Gromacs/Avg3merSARS2w" + x +"dt" + str(timestep[0]) +".xvg" for x in protein_list]
movie_pdb=["/gpfs/gpfs0/scratch/jws6pq/Gromacs/3merSARS2w" + x +"S1movie.pdb" for x in protein_list]
...
for protein_index, protein in enumerate(protein_list):
    ...
    for time in rmsd_time_steps:
        GromacsAnalysis.create_pdb_from_trajectory(tprfile[protein_index], newxtcfile[protein_index],
                                                   '/gpfs/gpfs0/scratch/jws6pq/Gromacs/SARSw' + protein +
                                                   'S1frame' + time + '.pdb', time)
        rmsd_change.append(GromacsAnalysis.pymol_rmsd('/gpfs/gpfs0/scratch/jws6pq/Gromacs/SARSw' + protein + 'S1frame' + time + '.pdb',
                                                      '/gpfs/gpfs0/scratch/jws6pq/Gromacs/SARSw' + protein + 'S1frame0.pdb'))
